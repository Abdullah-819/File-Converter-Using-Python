...
def convert_pdf_to_docx(input_path, output_path, progress_callback=None):
    ...
    for i in range(num_pages):
        converter.convert(output_path, start=i, end=i+1)
        if progress_callback:
            progress_callback(int((i+1)/num_pages*100))
    converter.close()
...
class FileConverterApp:
    ...
    def _convert_file(self):
        ...
        try:
            ...
            func(self.file_path, save_path, progress_callback=lambda p: self.root.after(0, lambda: self.circular_progress.update_progress(p)))
            messagebox.showinfo("Success",f"Conversion complete!\nSaved as:\n{save_path}")
        except Exception as e:
            messagebox.showerror("Error",str(e))
        finally:
